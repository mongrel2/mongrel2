...
a route, you write something like \file{/images/(.*\char`\\.jpg)} and the part before the `(' is used as a fast matched
...
other than to delimit where a pattern starts.  So instead of \shell{/images/.*\char`\\.jpg}, write \shell{/images/(.*\char`\\.jpg)}
for it to work.
...
\item \verb|"/images/(.*\.jpg)"| Match only requests for jpg images in the images directory.  Keep in mind that this
    isn't actually looking in the directory, it's just matching the \ident{(.*\char`\\.jpg)} pattern.
...
\item Mongrel2 loads these and converts them to PREFIX/PATTERN pairs.  For the first one
    the \verb|PREFIX="images"|, \verb|PATTERN="(.*\.jpg)"|.  For the second one, it's
    \verb|PREFIX="/users"| and \verb|PATTERN=None|.
