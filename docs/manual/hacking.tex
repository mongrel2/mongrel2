...
who thinks the world should be a happy rainbow of Unicode demanded it be in there.  Unicode in
...
\item [TCP] Programmers think these are streamed sequential messages, so if
   ...
   some framing mechanism you wouldn't know where one messages ends or begins.
...
help you get a handle on it.  First the client:

\begin{code}{Simple Python ZeroMQ Client}
<< d['docs/manual/inputs/simple_python_zmq_client.py|pyg|l'] >>
\end{code}

And then then the server:

\begin{code}{Simple Python ZeroMQ Server}
<< d['docs/manual/inputs/simple_python_zmq_server.py|pyg|l'] >>
\end{code}
...
<< d['docs/manual/inputs/zmq_req_rep_client.py|pyg|l'] >>
\end{code}


\begin{code}{ZeroMQ REQ/REP Server}
<< d['docs/manual/inputs/zmq_req_rep_server.py|pyg|l'] >>
\end{code}
