...
\verb|curl -T tests/config.sqlite http://localhost:6767/handlertes| to upload a big file.
...
\item Finally, you get a final request message that has both the \ident{X-Mongrel2-Upload-Start} and \ident{X-Mongrel2-Upload-Done} headers, which you can then use to read the finaly upload contents.  You should also make sure the headers match to prevent someone forging completed uploads.
\end{enumerate}
