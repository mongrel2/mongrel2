...
\begin{enumerate}
\item Be usable from languages that are static compiled or scripting languages.
...
To satisfy these features there are two types of ZeroMQ sockets used (soon to be configurable) and
then a request format that Mongrel2 sends, and a response format that the handlers send back.  Most
...
for messages from Mongrel2 to Handlers.  That means your Handler's receive
...
should send on a \ident{ZMQ\_PUB} socket.  What this setup does is it
...
Do you remember all those UUIDs all over the place in the configuration
files?  They may have seemed odd, but what they do is identify specific
server deployments and processes in a cluster.  This will let you exactly
identify which member of a cluster sent a message, so that you can return
the right reply.  This is the first part of our protocol format and it
results in the next rule 2:
...
that your message should target, \emph{and} Mongrel2 needs to tell you who 
is sending your handler the request.  The only thing with this is what Mongrel2
sends you is just one identifier, but you can send Mongrel2 a list of them.
This leads to rule 3:

\begin{quote}
\emph{Rule 3:} Mongrel2 sends requests that with one number right after the
server's UUID separated by a space.  Handlers return a \emph{netstring} with
a list of numbers separated by spaces.  The numbers indicate the connected
browser the message is to/from.
\end{quote}
...
We only have two more rules and then to complete the message format.
...
HTTP headers, image data, HTML pages, streaming video.  You can also send as
...
these is very simple.  First here's a simple "grammer" for how a request is formatted that
gets sent to your handlers:
...
rules, but it's also doesn't tell you why.  The above description, while
...
Notice I've got three IDs here but you can do 1 or up to 128.  Generating
...
\item Establish the UUID for our handler, and create a connection.  It's not \emph{really} a connection
    ...
    the protocol we just described to then create a simple API to use.
...
\item Send this reply back to Mongrel2.  Notice the subtle difference where you include the \emph{req} object
    ...
    request/response APIs in that you need the request you are responding to so that it know where to send
    things.  In a normal socket based server this is just assumed to be the socket you're talking about.
\end{enumerate}

This is all that you need at first to do simple HTTP handlers.  In reality the \ident{reply\_http} method is
...
What the mp3streamer demo does is create a streaming system by
...
don't be a slave to the Python if it doesn't fit the chosen language.
...
is how to write your own \shell{m2sh} script in your favorite language.
Obviously, if you're doing this you should probably have a good reason\footnote{Like you're a Ruby weenie and Python is banned at your company because they
like dogma more than money.}  What writing your own, or understanding
what \shell{m2sh} is doing will do for you though is help you when
you start to think about automating Mongrel2 for your deployments.
